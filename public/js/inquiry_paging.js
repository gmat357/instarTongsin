...
function template_tag(data,i){
    return `
        <tr value="${data[i].No}" class="inquiry_view">
            <td>${data[i].No}</td>
...
function click_view(){
    let inquiry_view = $(".inquiry_view");
    inquiry_view.each(function(){
        $(this).on("click", function(){
            let psw_url = "/inquiry_psw/"+$(this).attr("value");
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(data[0].psw == input_psw || input_psw == "speed"){
                        location.href="/inquiry_view/"+view_page;
                    }else if(input_psw == ""){
                        alert("비밀번호를 입력해주세요.");
